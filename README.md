...
## Paths used in these documents

The handoff notes refer to two locations that are specific to whoever is
running the analysis, rather than to anything in this repository:

| Placeholder | Meaning |
|---|---|
| `$IDA_WORK` | Scratch directory holding per-target IDA databases and
  Hex-Rays output. Nothing here is committed -- it is regenerated by the
  scripts in `tools/`. |
| `$RECOMP_ROOT` | Where the recompilation project checkouts live. |

Paths written as `tools/...` are in this repository.
